...
const VideoCall = ({sessionId, token}:VideoCallProps) => {
    ...
    try {
        ...
        sessionRef.current!.on("sessionConnected", ()=>{
            ...
            (error:any)=>{
                    if(error){
                        ...
                        console.log("publisher initialized successfully - you should see you video now")
                    }
                }
        );
        });
        ...
        sessionRef.current!.connect(token, (error:any)=>{
            if(error){
                toast.error("Error connecting to video");
            }else{
                if(publisherRef.current){
                    sessionRef.current!.publish(publisherRef.current, (error:any)=>{
                        if(error){
        ...
        toast.error("Failed to connect video call", error.message);
        setIsLoading(false);
    }
    }
                ...
                <div className='space-y-6'>
                    ...
                    <div className='text-center'>
                        ...
                        <p className='text-muted-foreground text-sm mt-1'>
                            After your consultation, click the red button to end the call.
                        </p>
                    </div>
                </div>
            )}
